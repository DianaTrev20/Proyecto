...
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml') #Carga un modelo Haar Cascade preentrenado para detectar rostros en imágenes.

captura = cv2.VideoCapture(0) #inicia la camara web
...
@app.route('/')#Renderiza index.html, mostrando los usuarios disponibles.
def index():
    return render_template("indexImg.html", usuarios=usuarios)
